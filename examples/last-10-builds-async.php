...
$repository = $client->repository($argv[1] ?? 'WyriHaximus/php-travis-client')
    ->doOnNext(function (Repository $repository) {
        //Side effects should be limited to `do` and `subscribe`
...
$builds = $repository->flatMap(function (Repository $repository) {
    return $repository->builds();
});

$builds->subscribe(new CallbackObserver(function (BuildInterface $build) {
    echo "\t", 'Build', PHP_EOL;
